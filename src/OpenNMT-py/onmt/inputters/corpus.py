...
from torchtext.data import Dataset as TorchtextDataset, Example as TorchtextExample
...
class DatasetAdapter(object):
    ...
    def _valid_fields(cls, fields):
        ...
        return {f_k: f_v for f_k, f_v in fields.items() if f_k in cls.valid_field_name}
    ...
    def _process(item, is_train):
        ...
        maybe_example = transform.apply(example, is_train=is_train, corpus_name=cid)
    ...
    def _maybe_add_dynamic_dict(self, example, fields):
        ...
        if "src_map" in fields and "alignment" in fields:
            example = _dynamic_dict(
                example, fields["src"].base_field, fields["tgt"].base_field
            )
        return example
    ...
    def _to_examples(self, bucket, is_train=False):
        ...
        for item in bucket:
            ...
            if maybe_example is not None:
                ...
                ex_fields = {
                    k: [(k, v)] for k, v in self.fields_dict.items() if k in example
                }
...
class ParallelCorpus(object):
    ...
    def load(self, offset=0, stride=1):
        ...
        with exfile_open(self.src, mode="rb") as fs, exfile_open(
            ...
            for i, (sline, tline, align, *features) in enumerate(
                zip(fs, ft, fa, *features_files)
            ):
                if (i % stride) == offset:
                    ...
                    if features:
                        ...
                        for j, feat in enumerate(features):
                            example["src_feats"][features_names[j]] = feat.decode(
                                "utf-8"
                            )
                    yield example
...
def append_features_to_example(example, features):
    ...
    toks = [f"{subword}￨{feat}" for subword, feat in zip(ex_toks, feat_toks)]
    return " ".join(toks)
