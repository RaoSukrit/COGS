...
class DynamicDatasetIter(object):
    ...
    @classmethod
    def from_opts(cls, corpora, transforms, fields, opts, is_train, stride=1, offset=0):
    ...
    def _bucketing(self):
        buckets = torchtext_batch(
            self.mixer, batch_size=self.bucket_size, batch_size_fn=None
        )
        yield from buckets
...
def build_dynamic_dataset_iter(
    fields, transforms_cls, opts, is_train=True, stride=1, offset=0
):
    ...
    return DynamicDatasetIter.from_opts(
        corpora, transforms, fields, opts, is_train, stride=stride, offset=offset
    )
