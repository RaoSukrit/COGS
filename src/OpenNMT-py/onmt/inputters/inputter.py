...
def _pad_vocab_to_multiple(vocab, multiple):
    ...
    padding_tokens = [
        "{}{}".format(DefaultTokens.VOCAB_PAD, i)
...
def _build_fv_from_multifield(multifield, counters, build_fv_kwargs, size_multiple=1):
    for name, field in multifield:
        _build_field_vocab(
            field, counters[name], size_multiple=size_multiple, **build_fv_kwargs[name]
        )
...
def _build_fields_vocab(
    ...
    build_fv_kwargs["src"] = dict(
        max_size=src_vocab_size, min_freq=src_words_min_frequency, specials=src_specials
    )
    build_fv_kwargs["tgt"] = dict(
        max_size=tgt_vocab_size, min_freq=tgt_words_min_frequency, specials=tgt_specials
    )
...
def _merge_field_vocabs(
    src_field, tgt_field, vocab_size, min_freq, vocab_size_multiple, specials
):
    ...
    all_specials = list(
        OrderedDict.fromkeys(tok for tok in init_specials + specials if tok is not None)
    )
    merged = sum([src_field.vocab.freqs, tgt_field.vocab.freqs], Counter())
    merged_vocab = Vocab(
        merged, specials=all_specials, max_size=vocab_size, min_freq=min_freq
    )
