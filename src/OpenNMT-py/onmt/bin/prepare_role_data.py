...
class EncoderRNN(nn.Module):
    ...
    def __init__(
        self, input_size=747, hidden_size=512, mytype="LSTM", n_layers=1, dropout_p=0.1
    ):
...
class DecoderRNN(nn.Module):
    ...
    def __init__(
        self, hidden_size=512, output_size=727, mytype="LSTM", n_layers=1, dropout_p=0.1
    ):
...
def run_encoder(batch, model, test_encoder=None):
    ...
    if test_encoder:
        ...
        test_encoder_outputs, test_encoder_hidden = test_encoder(test_encoder_input)
...
def load_model(opt):
    ...
    src_vocab_savepath = os.path.join(
        save_dir, f"{num_examples}_src_{opt.data_split}_vocab.json"
    )
    tgt_vocab_savepath = os.path.join(
        save_dir, f"{num_examples}_tgt_{opt.data_split}_vocab.json"
    )
...
def export_model_weights(
    ...
    state = {
        "encoder_state_dict": modified_encoder_state_dict,
    ...
    if not os.path.exists(os.path.join(save_dir, f"cogs_ckpt_{model_prefix}.pt")):
        ...
        print(
            f"SAVED MODEL WEIGHTS AT {os.path.join(save_dir, f'cogs_ckpt_{model_prefix}.pt')}"
        )
    else:
        print(
            f'CHECKPOINT ALREADY EXISTS AT {os.path.join(save_dir, f"cogs_ckpt_{model_prefix}.pt")}'
        )
...
def save_embd(
    ...
    if debug:
        ...
        print(
            f"test_hidden== enc_states = {torch.isclose(test_hidden, enc_states).all()}"
        )
    ...
    for idx, (src_text_seg, tgt_text_seg) in enumerate(text_input):
        ...
        if debug:
            ...
            print(
                f"embd == enc_states {torch.isclose(embd.unsqueeze(0).unsqueeze(0), enc_states).all()}"
            )
            print("*" * 50 + "\n")
