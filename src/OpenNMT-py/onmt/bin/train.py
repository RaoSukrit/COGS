...
from onmt.inputters.fields import build_dynamic_fields, save_fields, load_fields
...
def train(opt):
    ...
    train_process = partial(
        single_main, fields=fields, transforms_cls=transforms_cls, checkpoint=checkpoint
    )
    ...
    if opt.world_size > 1:
        ...
        for device_id in range(nb_gpu):
            ...
            procs.append(
                mp.Process(
                    target=consumer,
                    args=(train_process, opt, device_id, error_queue, q, semaphore),
                    daemon=True,
                )
            )
        ...
        for device_id in range(nb_gpu):
            ...
            producer = mp.Process(
                ...
                args=(train_iter, queues[device_id], semaphore, opt, device_id),
                daemon=True,
            )
            producers.append(producer)
            producers[device_id].start()
            logger.info(
                " Starting producer process pid: {}  ".format(producers[device_id].pid)
            )
