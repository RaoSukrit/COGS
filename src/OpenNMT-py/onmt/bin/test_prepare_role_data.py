...
class DecoderRNN(nn.Module):
    ...
    def __init__(self, hidden_size=512, output_size=723, mytype='LSTM', n_layers=1, dropout_p=0.1):
...
def decode(batch, encoder, decoder, src_vocab, tgt_vocab, use_attn=False, debug=True,
        ...
        ## OPENNMT
        enc_hidden, enc_outputs, src_lengths = model.encoder(
            src, src_lengths
        )
        ...
        for t in range(max_target_length):
            if use_attn:
                op_decoder_output, op_decoder_hidden, op_decoder_context, op_decoder_attention = decoder(op_decoder_input,
                                                                                                         op_decoder_context,
                                                                                                         op_decoder_hidden,
                                                                                                         enc_outputs)
            else:
                op_decoder_output, op_decoder_hidden, op_decoder_raw_scores  = decoder(op_decoder_input,
                                                                              op_decoder_hidden)
        ...
        return pred_text, tgt_text, src_text, op_pred_text, torch.isclose(encoder_hidden, enc_hidden).all(), torch.isclose(encoder_outputs, enc_outputs).all()
...
def run_inference(opt, out_file=None, debug=True):
    ...
    for shard_num, (src_shard, tgt_shard) in enumerate(shard_pairs):
        ...
        for batch_num, batch in tqdm(enumerate(data_iter)):
            pred_text, tgt_text, input_text, op_pred_text, hidden_ag, out_ag = decode(batch,
                                                     test_encoder,
                                                     test_decoder,
                                                     src_vocab,
                                                     tgt_vocab,
